...
After unzipping, open the `workforce_ai_suite` folder directly in Antigravity. Then use the built-in terminal:

```bash
python app.py
```

See `ANTIGRAVITY_README.md` and `AGENTS.md` for IDE/AI-agent guidance.

## One-command local launcher

For the easiest local test, run this from inside the unzipped project folder:

```bash
python app.py
```

The launcher will create a local `.venv`, install packages from `requirements.txt` when needed, start Streamlit, and open your browser at `http://localhost:8501`.

Useful options:

```bash
python app.py --reinstall   # force package reinstall
python app.py --port 8502   # use a different local port
python app.py --headless    # do not auto-open the browser
```
